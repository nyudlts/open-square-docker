...
// // v0 schema
// const solrCollection = 'open-square-metadata-v0'
// v1 schema
...
// NOTE: the JSON files do not have the newer schema
...
// TODO: connect to DLTS viwer API instead of reading from the metadata docs
// why? epub-metadata has not been updated in years, while deployed data on Viewer API has way more information as well as better quality detail in the author/contributor section
...
//     data.response.docs.forEach(publication => {
//       fetch(publication.uri).then(data => {
//         return data.json();
//       }).then(jsondata => {
//         // TODO: ? store this in an array?
...
// pubs.forEach(pub => {
//   // TODO: fetch the individual data of each book
//   try {
//     const indivResponse = await fetch(publication.uri)
...
// example used for multi insertion directly from the filesystem
...
// const dirEntry = Deno.readDir(dir).next
  try {
    ...
    // // Solution: flatten the contributors array of objects before inserting. no need for the field to be multivalued
    ...
    // //   role: string,
    // // }
    // const authors: contrib[] = doc.author.split(', ')
    // console.log(authors)

    // // create the full author object, unordered, for flattening into a string storage in solr6.6
    // const oa = [];
    ...
    // create a review string (for storage in a solr6.6 without nested docs)
  ...
  const stringifiedContribs = JSON.stringify(nonstringcontribs)
  ...
  const obj =    {
    ...
    // TODO: contributor string should be created before data is inserted, not in the front-end
    "contributorsInOrder": ["Steven H. Jaffe", "Eric Foner"],
    "contributorsAsASentence": "By Steven H. Jaffe, and Eric Foner",
    "dateBook": "2018-05-01",
